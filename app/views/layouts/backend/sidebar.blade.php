<ul class="page-sidebar-menu" data-auto-scroll="true" data-slide-speed="200">
	<li><a href="{{ URL::to('user/edit') }}"><span class="title"><i class="icon-user"></i> Edit profile</span></a></li>
	<li><a href="{{ URL::to('user/transactions') }}"><span class="title"><i class="icon-refresh"></i> User transactions</span></a></li>
	<li><a href="{{ URL::to('user/transactions') }}"><span class="title"><i class="icon-refresh"></i> Invest</span></a></li>
	<li><a href="{{ URL::to('user/transactions') }}"><span class="title"><i class="icon-refresh"></i> Add money</span></a></li>
	<li><a href="{{ URL::to('user/admin/transactions') }}"><span class="title">Admin transactions</span></a></li>
	<li><a href="{{ URL::to('logout') }}"><span class="title">Logout</span></a></li>
</ul>